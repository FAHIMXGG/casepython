...
export default function DownloadOrderPDF({
  ...
  material,
  finish,
}: DownloadOrderPDFProps) {
  const [isGenerating, setIsGenerating] = useState(false);

  const generatePDF = async () => {
    if (!designImageUrl) {
      alert("Design image not available for this order");
      return;
    }
    ...
    try {
      ...
      pdf.save(`Order-${orderId.slice(0, 8)}-${modelName}.pdf`);
    } catch (error) {
  ...
  return (
    <Button
      onClick={generatePDF}
      disabled={isGenerating || !designImageUrl}
      variant="ghost"
      size="sm"
